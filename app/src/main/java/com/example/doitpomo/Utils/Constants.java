package com.example.doitpomo.Utils;

public class Constants {

    public static final int DB_VERSION = 1;
    public static final String DB_NAME = "todoListDB";
    public static final String TABLE_NAME = "todoTBL";
    public static final String SUBTASK_TABLE_NAME = "subtasksTBL";

    //Table columns
    public static final String KEY_ID = "id";
    public static final String KEY_TODO_ITEM = "todo_item";
    public static final String KEY_DATE_NAME = "date_added";
    public static final String KEY_TIME_SPENT = "time_spent";
    public static final String KEY_PRIORITY= "priority";
    public static final String KEY_DESCRIPTION = "description";
    public static final String KEY_TODO_DATE_FINISH = "date_finish";
    public static final String KEY_DONE = "done";
    public static final String KEY_COMPLETION_DATE = "completion_date";
    public static final String PREFS_NAME = "myPrefsFile";
    public static final String KEY_TODO_ID = "todo_id_subtask";
    public static final String KEY_SUBTASK_ITEM = "subtask_item";
    public static final String KEY_SUBTASK_ID = "subtask_id";
    public static final String KEY_SUBTASK_DONE = "subtask_done";
}
